...
# set up data----
#Metadata
Plant.endo.dat <- read.csv("~/Documents/Mitchell Lab/Brandon/Summer 2018/Pool Experiment/Endo_Data_Cleaned.csv")
...
poolprev.dat <- read.csv("~/Documents/Mitchell Lab/Pool Experiment/Entered Data/FullPREV_2018_1.csv")  %>%
  filter(Dam == "D" | Dam == "ND") %>%
...
# restructure data
poolprev.long <- gather(poolprev.dat2, key = Parasite, value = Infected, Rhiz, Coll, Pucc, Sept) %>% 
...
resids.fig(Rmod.slopes, new2) #the the total worst?
...
resids.fig(Rmod.slopes3, new2) #better, homoscedastic
...
t.test(MaxRhiz.Prev~Pool_Endo, data=new3)

require(plotrix)
myData <- aggregate(new3$MaxRhiz.Prev,
                    by = list(cyl = new3$Pool_Endo),
